...
module.exports = function (ast, logicalPath, config) {
    ...
    /**
     * Whether the file currently being processd is the Aria singleton
     * @type Boolean
     */
    ...
    /**
     * Returns true if the given node is an instruction that logs an error ($logError/$logWarn/$logInfo/_logError )
     * @param {Object} input node from the ast
     */
    ...
    function checkIfBeanDefinition (input) {
        if (input instanceof UglifyJS.AST_SimpleStatement && input.body instanceof UglifyJS.AST_Call) {
            var fnRef = input.body.expression;

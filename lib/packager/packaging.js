 ...
 */
Packaging.prototype.expandLogicalPaths = function (patterns) {
    var file = grunt.file;
...
Packaging.prototype.getAbsolutePath = function (logicalPath) {
    ...
    var sourceDirectories = this.sourceDirectories;
    for (var i = 0, l = sourceDirectories.length; i < l; i++) {
        var absoluteFile = path.resolve(path.join(sourceDirectories[i], logicalPath));
...
Packaging.prototype.builtinVisitors = requireDirectory(__dirname, '../visitors');
